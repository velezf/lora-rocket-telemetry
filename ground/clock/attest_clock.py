...
mean no ingest = lost flight. After the operator has set the clock by hand
(`sudo date -s '<YYYY-MM-DD HH:MM:SS>'`), this drops the `/run/apogee-rtc-restored`
marker so the gate trusts the clock and apogee-ingest will start. Audit to STDOUT only.
