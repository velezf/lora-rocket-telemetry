...
- **8.4** ✅ Guess-the-apogee — BUILT + button-flow bench-tested with Frank 2026-08-31 (`app/game.py`). **Startup asks which game first**: `GAME? Closest`/`NoOver` (#5/#6 toggle, #12 confirms; the `RULE` knob is the preselection). Then the screen prompts each kid **by name** (`Bacon? 350ft`), #5 up / #6 down, and **#12 twice** — first shows `Bacon 350 OK?`, second banks it and seats the next kid (dialing during the OK? cancels). Liftoff closes betting (an unlocked dial still banks), the apogee screen names the winner (ties → earlier in the roster), and a sled power-cycle resets to the game pick. Display labels everything: `ALT` before the hero number, RSSI top-right as `-44dBm`, bottom row lifted clear of the screen edge.
