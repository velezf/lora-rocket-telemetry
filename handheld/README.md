...
- **8.4** ✅ Guess-the-apogee — BUILT 2026-08-31 and **family-tested the same day through three live UX rounds, final flow + start-over chord verified on the bench** (`app/game.py`). **Startup asks which game first**: `GAME? Closest`/`NoOver` (#5/#6 toggle, #12 confirms; the `RULE` knob is the preselection). Then the screen prompts each kid **by name** (`Bacon? 350ft`), #5 up / #6 down, and **#12 twice** — first shows `Bacon 350 OK?`, second banks it and seats the next kid (dialing during the OK? cancels). Once every kid locks in, the screen shows **`LOCKED x3 #12=play`** and holds armed until launch — and **#12 from there opens a RocketLab side quest**: the real-flight guesses are stashed, any number of lab rounds can run while waiting, a real liftoff instantly restores the armed game (original guesses intact, scored at the real apogee), and the #5+#6 chord leaves the quiz back to armed (chord again from armed = full start-over, rule kept). Liftoff closes betting (an unlocked dial still banks), the apogee screen names the winner (ties → earlier in the roster), and a sled power-cycle AFTER a flight resets to the game pick — plain pad frames never do (the bounce-back-to-menu bug, fixed same day). Display labels everything: `ALT` before the hero number, RSSI top-right as `-44dBm`, bottom row lifted clear of the screen edge.
- **8.5** Kid-tweakable messages + rules — the knobs (`PLAYERS`, `RULE`, `STEP_FT`, `START_FT`) sit at the top of `app/game.py` with instructions; edit + service restart. `RULE`: `"closest"` (absolute distance) or `"no-over"` (Price-Is-Right — busting loses; everyone busts, the rocket wins); it PRESELECTS the startup menu rather than fixing the choice
- **RocketLab** (3rd game, BUILT 2026-08-31) — offline range-downtime quiz: pick a rocket (name + **mass/diameter shown**), pick a motor (code + certified impulse/avg thrust — the code lies a little, an A8 averages ~3 N), kids guess by name, and a deterministic 1-DOF sim (`app/rocketsim.py`) is the single answer authority — **calibrated against the Katana's real 859 ft flight (432 g weighed) to within ~1 %**. Dedicated screens, no ALT/dBm. **A real liftoff preempts the quiz.** Tables in `app/rockets.py` are kid-extendable; sim knobs (`CD`, `RHO`) top of `rocketsim.py`
